...
# 34 "./src/main/navigation/navigation_pos_estimator.c" 2
...
# 40 "./src/main/navigation/navigation_pos_estimator.c" 2
...
# 44 "./src/main/navigation/navigation_pos_estimator.c" 2
...
# 48 "./src/main/navigation/navigation_pos_estimator.c" 2
...
# 25 "./src/main/navigation/navigation_private.h" 2
...
# 54 "./src/main/navigation/navigation_pos_estimator.c" 2
...
# 55 "./src/main/navigation/navigation_pos_estimator.c" 2
...
# 23 "./src/main/drivers/rangefinder.h" 2
...
# 56 "./src/main/navigation/navigation_pos_estimator.c" 2
# 64 "./src/main/navigation/navigation_pos_estimator.c"
...
#define INAV_HISTORY_BUF_SIZE (INAV_POSITION_PUBLISH_RATE_HZ / 2)
...
static _Bool updateTimer(navigationTimer_t * tim, timeUs_t interval, timeUs_t currentTimeUs)
{
...
static _Bool shouldResetReferenceAltitude(void)
{
...
# 255 "./src/main/navigation/navigation_pos_estimator.c"
...
void onNewGPSData(void)
{
    ...
    static _Bool isFirstGPSUpdate = 1;
        ...
        if (posControl.gpsOrigin.valid) {
            ...
            if (!isFirstGPSUpdate) {
                ...
                if (positionEstimationConfig()->use_gps_velned && gpsSol.flags.validVelNE) {
                    ...
                    posEstimator.gps.vel.V.X = (posEstimator.gps.vel.V.X + (1.113195f * (gpsSol.llh.lat - previousLat) / dT)) / 2.0f;
                    posEstimator.gps.vel.V.Y = (posEstimator.gps.vel.V.Y + (gpsScaleLonDown * 1.113195f * (gpsSol.llh.lon - previousLon) / dT)) / 2.0f;
                }

                if (positionEstimationConfig()->use_gps_velned && gpsSol.flags.validVelD) {
                    posEstimator.gps.vel.V.Z = - gpsSol.velNED[2];
                }
                else {
...
# 490 "./src/main/navigation/navigation_pos_estimator.c"
static void updateIMUTopic(void)
{
    ...
    if (!isImuReady()) {
        ...
        posEstimator.imu.gravityCalibrationComplete = 0;
    }
    else {
        t_fp_vector accelBF;
...
static void updateEstimatedTopic(timeUs_t currentTimeUs)
{
    ...
    float dt = ((currentTimeUs - posEstimator.est.lastUpdateTime) * 1e-6f);
    ...
    _Bool isGPSValid = sensors(SENSOR_GPS) &&
    ...
    _Bool isBaroValid = sensors(SENSOR_BARO) && ((currentTimeUs - posEstimator.baro.lastUpdateTime) <= ((200) * 1000));
    _Bool isSurfaceValid = sensors(SENSOR_RANGEFINDER) && ((currentTimeUs - posEstimator.surface.lastUpdateTime) <= ((300) * 1000));


    if (isBaroValid) {
        if (!(armingFlags & (ARMED))) {
            posEstimator.state.baroGroundAlt = posEstimator.est.pos.V.Z;
            posEstimator.state.isBaroGroundValid = 1;
            ...
            if (posEstimator.est.vel.V.Z > 15) {
                if (currentTimeUs > posEstimator.state.baroGroundTimeout) {
                    posEstimator.state.isBaroGroundValid = 0;
                }
            }
            else {
                posEstimator.state.baroGroundTimeout = currentTimeUs + 250000;
    ...
    _Bool isAirCushionEffectDetected = (armingFlags & (ARMED)) &&
    ...
    const _Bool isGPSZValid = isGPSValid && (posEstimator.gps.epv < positionEstimationConfig()->max_eph_epv);
    const _Bool useGpsZPos = (stateFlags & (FIXED_WING)) && !sensors(SENSOR_BARO) && isGPSValid && isGPSZValid;
    const _Bool useGpsZVel = isGPSValid && isGPSZValid;


    const _Bool isEstXYValid = (posEstimator.est.eph < positionEstimationConfig()->max_eph_epv);
    const _Bool isEstZValid = (posEstimator.est.epv < positionEstimationConfig()->max_eph_epv);


    if (isGPSValid) {
        _Bool positionWasReset = 0;
    ...
    int gpsHistoryIndex = (posEstimator.history.index - 1) - constrain(((int)positionEstimationConfig()->gps_delay_ms / (1000 / 50)), 0, (50 / 2) - 1);
    if (gpsHistoryIndex < 0) {
        gpsHistoryIndex += (50 / 2);
    }
    ...
    if (isGPSValid) {
        ...
        gpsResidualXYMagnitude = sqrtf(((gpsResidual[X][0])*(gpsResidual[X][0])) + ((gpsResidual[Y][0])*(gpsResidual[Y][0])));
    }
    ...
    if (useGpsZPos || isBaroValid) {
        ...
        if (useGpsZPos) {
            ...
            newEPV = updateEPE(posEstimator.est.epv, dt, ( __extension__ ({ __typeof__(posEstimator.gps.epv) _left2 = (posEstimator.gps.epv); __typeof__(gpsResidual[Z][0]) _right3 = (gpsResidual[Z][0]); _left2 > _right3 ? _left2 : _right3; })), positionEstimationConfig()->w_z_gps_p);
        }


        if (useGpsZVel) {
            inavFilterCorrectVel(Z, dt, gpsResidual[Z][1], positionEstimationConfig()->w_z_gps_v * ((gpsWeightScaler)*(gpsWeightScaler)));
        }
    }
    else {
    ...
    if (isGPSValid) {
        ...
        newEPH = updateEPE(posEstimator.est.eph, dt, ( __extension__ ({ __typeof__(posEstimator.gps.eph) _left4 = (posEstimator.gps.eph); __typeof__(gpsResidualXYMagnitude) _right5 = (gpsResidualXYMagnitude); _left4 > _right5 ? _left4 : _right5; })), positionEstimationConfig()->w_xy_gps_p);
    }
    else {
    ...
    if (positionEstimationConfig()->w_acc_bias > 0) {
        ...
        if (isGPSValid) {
            accelBiasCorr.V.X -= gpsResidual[X][0] * ((positionEstimationConfig()->w_xy_gps_p)*(positionEstimationConfig()->w_xy_gps_p));
            accelBiasCorr.V.Y -= gpsResidual[Y][0] * ((positionEstimationConfig()->w_xy_gps_p)*(positionEstimationConfig()->w_xy_gps_p));

            if (useGpsZPos) {
                accelBiasCorr.V.Z -= gpsResidual[Z][0] * ((positionEstimationConfig()->w_z_gps_p)*(positionEstimationConfig()->w_z_gps_p));
            }
        }


        if (isBaroValid && !isAirCushionEffectDetected) {
            accelBiasCorr.V.Z -= baroResidual * ((positionEstimationConfig()->w_z_baro_p)*(positionEstimationConfig()->w_z_baro_p));
        }

        const float accelBiasCorrMagnitudeSq = ((accelBiasCorr.V.X)*(accelBiasCorr.V.X)) + ((accelBiasCorr.V.Y)*(accelBiasCorr.V.Y)) + ((accelBiasCorr.V.Z)*(accelBiasCorr.V.Z));
...
# 826 "./src/main/navigation/navigation_pos_estimator.c"
    posEstimator.est.surface = -1;
    posEstimator.est.surfaceVel = 0;
    posEstimator.est.surfaceValid = 0;
...
static void publishEstimatedTopic(timeUs_t currentTimeUs)
{
    ...
    if (updateTimer(&posPublishTimer, (1000000 / (50)), currentTimeUs)) {
        ...
        if (posEstimator.est.eph < positionEstimationConfig()->max_eph_epv) {
            updateActualHorizontalPositionAndVelocity(1, posEstimator.est.pos.V.X, posEstimator.est.pos.V.Y, posEstimator.est.vel.V.X, posEstimator.est.vel.V.Y);
        }
        else {
            updateActualHorizontalPositionAndVelocity(0, posEstimator.est.pos.V.X, posEstimator.est.pos.V.Y, 0, 0);
        }


        if (posEstimator.est.epv < positionEstimationConfig()->max_eph_epv) {
            updateActualAltitudeAndClimbRate(1, posEstimator.est.pos.V.Z, posEstimator.est.vel.V.Z);
        }
        else {
            updateActualAltitudeAndClimbRate(0, posEstimator.est.pos.V.Z, 0);
        }
        ...
        if (posEstimator.history.index >= (50 / 2)) {
            posEstimator.history.index = 0;
        }
