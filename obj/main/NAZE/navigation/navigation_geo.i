...
# 136 "./src/main/navigation/navigation_geo.c"
void geoSetOrigin(gpsOrigin_s * origin, const gpsLocation_t * llh, geoOriginResetMode_e resetMode)
{
...
{
    if (origin->valid) {
        pos->V.X = (llh->lat - origin->lat) * 1.113195f;
        pos->V.Y = (llh->lon - origin->lon) * (1.113195f * origin->scale);
...
void geoConvertLocalToGeodetic(const gpsOrigin_s * origin, const t_fp_vector * pos, gpsLocation_t * llh)
{
    ...
    llh->lat += lrintf(pos->V.X / 1.113195f);
    llh->lon += lrintf(pos->V.Y / (1.113195f * scaleLonDown));
