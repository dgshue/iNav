...
{
    const float swingVelocity = (( __extension__ ({ __typeof__(imuMeasuredRotationBF.A[Z]) _abs0 = (imuMeasuredRotationBF.A[Z]); _abs0 < 0 ? -_abs0 : _abs0; })) > ((100) * (3.14159265358979323846f / 180.0f))) ? (imuMeasuredAccelBF.A[Y] / imuMeasuredRotationBF.A[Z]) : 0;
    const _Bool isForwardAccelerationHigh = (imuMeasuredAccelBF.A[X] > navConfig()->fw.launch_accel_thresh);
    const _Bool isAircraftAlmostLevel = (calculateCosTiltAngle() >= cos_approx(((navConfig()->fw.launch_max_angle) * (3.14159265358979323846f / 180.0f))));

    const _Bool isBungeeLaunched = isForwardAccelerationHigh && isAircraftAlmostLevel;
    const _Bool isSwingLaunched = (swingVelocity > navConfig()->fw.launch_velocity_thresh) && (imuMeasuredAccelBF.A[X] > 0);
...
void resetFixedWingLaunchController(timeUs_t currentTimeUs)
{
    ...
    launchState.launchDetected = 0;
    launchState.launchFinished = 0;
    launchState.motorControlAllowed = 0;
}
...
void enableFixedWingLaunchController(timeUs_t currentTimeUs)
{
    ...
    launchState.motorControlAllowed = 1;
}
...
void applyFixedWingLaunchController(timeUs_t currentTimeUs)
{
    ...
    if (launchState.launchDetected) {
        ...
        if (launchState.motorControlAllowed) {
            ...
            if ((( __extension__ ({ __typeof__(rcCommand[ROLL]) _abs1 = (rcCommand[ROLL]); _abs1 < 0 ? -_abs1 : _abs1; })) > rcControlsConfig()->pos_hold_deadband) || (( __extension__ ({ __typeof__(rcCommand[PITCH]) _abs2 = (rcCommand[PITCH]); _abs2 < 0 ? -_abs2 : _abs2; })) > rcControlsConfig()->pos_hold_deadband)) {
                launchState.launchFinished = 1;
            }
            ...
            if (timeElapsedSinceLaunchMs >= navConfig()->fw.launch_motor_timer) {
                ...
                if (navConfig()->fw.launch_motor_spinup_time > 0) {
                    ...
                    const uint16_t minIdleThrottle = ( __extension__ ({ __typeof__(motorConfig()->minthrottle) _left3 = (motorConfig()->minthrottle); __typeof__(navConfig()->fw.launch_idle_throttle) _right4 = (navConfig()->fw.launch_idle_throttle); _left3 > _right4 ? _left3 : _right4; }));
