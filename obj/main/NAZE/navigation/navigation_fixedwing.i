...
static _Bool isPitchAdjustmentValid = 0;
static _Bool isRollAdjustmentValid = 0;
...
static void updateAltitudeVelocityAndPitchController_FW(timeDelta_t deltaMicros)
{
    ...
    const float demSPE = (posControl.desiredState.pos.V.Z / 100.0f) * 9.80665f;
    const float demSKE = 0.0f;

    const float estSPE = (posControl.actualState.pos.V.Z / 100.0f) * 9.80665f;
...
static void calculateVirtualPositionTarget_FW(float trackingPeriod)
{
    ...
    float trackingDistance = trackingPeriod * ( __extension__ ({ __typeof__(posControl.actualState.velXY) _left0 = (posControl.actualState.velXY); __typeof__(100.0f) _right1 = (100.0f); _left0 > _right1 ? _left0 : _right1; }));


#define TAN_15DEG 0.26795f
    _Bool needToCalculateCircularLoiter = isApproachingLastWaypoint()
    ...
    if (posControl.flags.isAdjustingPosition) {
        ...
        if (rcRollAdjustment) {
            ...
            posControl.flags.isAdjustingPosition = 1;
        }
    }
}
...
static void updatePositionHeadingController_FW(timeUs_t currentTimeUs, timeDelta_t deltaMicros)
{
    ...
    static _Bool errorIsDecreasing;
    static _Bool forceTurnDirection = 0;
    ...
    if (( __extension__ ({ __typeof__(headingError) _abs2 = (headingError); _abs2 < 0 ? -_abs2 : _abs2; })) > 17000) {
        forceTurnDirection = 1;
    }
    else if (( __extension__ ({ __typeof__(headingError) _abs3 = (headingError); _abs3 < 0 ? -_abs3 : _abs3; })) < 9000 && forceTurnDirection) {
        forceTurnDirection = 0;
    }


    if (forceTurnDirection) {
        headingError = ( __extension__ ({ __typeof__(headingError) _abs4 = (headingError); _abs4 < 0 ? -_abs4 : _abs4; }));
    }


    if ((currentTimeUs - previousTimeMonitoringUpdate) >= (1000000 / (2))) {

        errorIsDecreasing = (( __extension__ ({ __typeof__(previousHeadingError) _abs5 = (previousHeadingError); _abs5 < 0 ? -_abs5 : _abs5; })) > ( __extension__ ({ __typeof__(headingError) _abs6 = (headingError); _abs6 < 0 ? -_abs6 : _abs6; })));
    ...
    float rollAdjustment = navPidApply2(&posControl.pids.fw_nav, posControl.actualState.yaw + headingError, posControl.actualState.yaw, ((deltaMicros) * 1e-6f),
    ...
    if (posControl.flags.hasValidPositionSensor) {
        ...
        if (posControl.flags.horizontalPositionDataNew) {
            ...
            if (deltaMicrosPositionUpdate < (1000000 / (5))) {
                ...
                if (( __extension__ ({ __typeof__(posControl.actualState.velXY - 700.0f) _abs7 = (posControl.actualState.velXY - 700.0f); _abs7 < 0 ? -_abs7 : _abs7; })) > 50) {
...
void applyFixedWingPitchRollThrottleController(navigationFSMStateFlags_t navStateFlags, timeUs_t currentTimeUs)
{
    ...
    if (isPitchAdjustmentValid && (navStateFlags & NAV_CTL_ALT)) {
        ...
        throttleCorrection += ((pitchCorrection) / 10) * navConfig()->fw.pitch_to_throttle;
# 440 "./src/main/navigation/navigation_fixedwing.c"
    ...
    if (isPitchAdjustmentValid && (navStateFlags & NAV_CTL_ALT)) {
        ...
        pitchCorrection = constrain(pitchCorrection, -((navConfig()->fw.max_dive_angle) * 10), ((navConfig()->fw.max_climb_angle) * 10));
        rcCommand[PITCH] = -pidAngleToRcCommand(pitchCorrection, pidProfile()->max_angle_inclination[FD_PITCH]);
    }

    if (isRollAdjustmentValid && (navStateFlags & NAV_CTL_POS)) {
        rollCorrection = constrain(rollCorrection, -((navConfig()->fw.max_bank_angle) * 10), ((navConfig()->fw.max_bank_angle) * 10));
...
# 498 "./src/main/navigation/navigation_fixedwing.c"
}
...
# 567 "./src/main/navigation/navigation_fixedwing.c"
