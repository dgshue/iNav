...
static _Bool prepareForTakeoffOnReset = 0;
...
static void updateAltitudeVelocityController_MC(timeDelta_t deltaMicros)
{
    ...
    if (( __extension__ ({ __typeof__(targetVel) _abs0 = (targetVel); _abs0 < 0 ? -_abs0 : _abs0; })) > ( __extension__ ({ __typeof__(posControl.desiredState.vel.V.Z) _abs1 = (posControl.desiredState.vel.V.Z); _abs1 < 0 ? -_abs1 : _abs1; }))) {
...
static void updateAltitudeThrottleController_MC(timeDelta_t deltaMicros)
{
    ...
    posControl.rcAdjustment[THROTTLE] = navPidApply2(&posControl.pids.vel[Z], posControl.desiredState.vel.V.Z, posControl.actualState.vel.V.Z, ((deltaMicros) * 1e-6f), thrAdjustmentMin, thrAdjustmentMax, 0);
...
void resetMulticopterAltitudeController(void)
{
    ...
    if (prepareForTakeoffOnReset) {
        ...
        prepareForTakeoffOnReset = 0;
    }
    else {
        posControl.desiredState.vel.V.Z = posControl.actualState.vel.V.Z;
...
static void updatePositionAccelController_MC(timeDelta_t deltaMicros, float maxAccelLimit)
{
    ...
    const float newAccelX = navPidApply2(&posControl.pids.vel[X], posControl.desiredState.vel.V.X, posControl.actualState.vel.V.X, ((deltaMicros) * 1e-6f), accelLimitXMin, accelLimitXMax, 0);
    const float newAccelY = navPidApply2(&posControl.pids.vel[Y], posControl.desiredState.vel.V.Y, posControl.actualState.vel.V.Y, ((deltaMicros) * 1e-6f), accelLimitYMin, accelLimitYMax, 0);
...
static void applyMulticopterPositionController(timeUs_t currentTimeUs)
{
    ...
    const timeDelta_t deltaMicros = currentTimeUs - previousTimeUpdate;
    previousTimeUpdate = currentTimeUs;
    _Bool bypassPositionController;
...
_Bool isMulticopterLandingDetected(void)
{
    ...
    _Bool verticalMovement = fabsf(posControl.actualState.vel.V.Z) > 25.0f;


    _Bool horizontalMovement = posControl.actualState.velXY > 100.0f;





    _Bool isAtMinimalThrust = 0;
    ...
    _Bool possibleLandingDetected = isAtMinimalThrust && !verticalMovement && !horizontalMovement;
    ...
    if (!possibleLandingDetected) {
        ...
        return ((currentTimeUs - landingTimer) > (navConfig()->mc.auto_disarm_delay * 1000)) ? 1 : 0;
    }
}
