...
static _Bool motorLimitReached = 0;
...
const mixerConfig_t pgResetTemplate_mixerConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .mixerMode = MIXER_QUADX, .yaw_motor_direction = 1, .yaw_jump_prevention_limit = 200 }
...
extern const motorConfig_t pgResetTemplate_motorConfig; motorConfig_t motorConfig_System; motorConfig_t motorConfig_Copy; extern const pgRegistry_t motorConfig_Registry; const pgRegistry_t motorConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 6 | (0 << 12), .size = sizeof(motorConfig_t) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&motorConfig_System, .copy = (uint8_t*)&motorConfig_Copy, .ptr = 0, .reset = {.ptr = (void*)&pgResetTemplate_motorConfig}, };

const motorConfig_t pgResetTemplate_motorConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .minthrottle = 1150, .motorPwmProtocol = PWM_TYPE_STANDARD, .motorPwmRate = 400, .maxthrottle = 1850, .mincommand = 1000 }
...
#define DEF_MIXER(_mixerMode,_flyingPlatformType,_motorCount,_useServos,_hasFlaps,_motorMap) { .mixerMode=_mixerMode, .flyingPlatformType=_flyingPlatformType, .motorCount=_motorCount, .useServos=_useServos, .hasFlaps=_hasFlaps, .motor=_motorMap }
...
static const mixer_t mixerTable[] = {
    ...
    { .mixerMode=MIXER_TRI, .flyingPlatformType=PLATFORM_MULTIROTOR, .motorCount=3, .useServos=1, .hasFlaps=0, .motor=mixerTricopter },
    { .mixerMode=MIXER_CUSTOM_TRI, .flyingPlatformType=PLATFORM_MULTIROTOR, .motorCount=3, .useServos=0, .hasFlaps=0, .motor=((void *)0) },

    { .mixerMode=MIXER_QUADP, .flyingPlatformType=PLATFORM_MULTIROTOR, .motorCount=4, .useServos=0, .hasFlaps=0, .motor=mixerQuadP },
    { .mixerMode=MIXER_QUADX, .flyingPlatformType=PLATFORM_MULTIROTOR, .motorCount=4, .useServos=0, .hasFlaps=0, .motor=mixerQuadX },


    { .mixerMode=MIXER_HEX6X, .flyingPlatformType=PLATFORM_MULTIROTOR, .motorCount=6, .useServos=0, .hasFlaps=0, .motor=mixerHex6X },


    { .mixerMode=MIXER_CUSTOM, .flyingPlatformType=PLATFORM_MULTIROTOR, .motorCount=0, .useServos=0, .hasFlaps=0, .motor=((void *)0) },




    { .mixerMode=MIXER_FLYING_WING, .flyingPlatformType=PLATFORM_AIRPLANE, .motorCount=2, .useServos=1, .hasFlaps=0, .motor=mixerDualProp },
    { .mixerMode=MIXER_AIRPLANE, .flyingPlatformType=PLATFORM_AIRPLANE, .motorCount=2, .useServos=1, .hasFlaps=1, .motor=mixerDualProp },
    { .mixerMode=MIXER_CUSTOM_AIRPLANE, .flyingPlatformType=PLATFORM_AIRPLANE, .motorCount=2, .useServos=1, .hasFlaps=1, .motor=((void *)0) },
# 275 "./src/main/flight/mixer.c"
};
...
void mixerUsePWMIOConfiguration(void)
{
    ...
    if (currentMixerMode == MIXER_CUSTOM || currentMixerMode == MIXER_CUSTOM_TRI || currentMixerMode == MIXER_CUSTOM_AIRPLANE) {
        ...
        motorCount = ( __extension__ ({ __typeof__(mixer->motorCount) _left0 = (mixer->motorCount); __typeof__(pwmGetOutputConfiguration()->motorCount) _right1 = (pwmGetOutputConfiguration()->motorCount); _left0 < _right1 ? _left0 : _right1; }));
...
void mixTable(void)
{
    ...
    if ((stateFlags & (FIXED_WING)) && (flightModeFlags & (PASSTHRU_MODE))) {
        ...
        if (motorCount >= 4 && mixerConfig()->yaw_jump_prevention_limit < 500) {
            ...
            input[YAW] = constrain(input[YAW], -mixerConfig()->yaw_jump_prevention_limit - ( __extension__ ({ __typeof__(rcCommand[YAW]) _abs2 = (rcCommand[YAW]); _abs2 < 0 ? -_abs2 : _abs2; })), mixerConfig()->yaw_jump_prevention_limit + ( __extension__ ({ __typeof__(rcCommand[YAW]) _abs3 = (rcCommand[YAW]); _abs3 < 0 ? -_abs3 : _abs3; })));
        }
    }
    ...
    if (feature(FEATURE_3D)) {
        ...
        if ((rcCommand[THROTTLE] <= (rxConfig()->midrc - rcControlsConfig()->deadband3d_throttle))) {
            throttleMax = flight3DConfig()->deadband3d_low;
            throttleMin = motorConfig()->minthrottle;
            throttlePrevious = throttleCommand = rcCommand[THROTTLE];
        } else if (rcCommand[THROTTLE] >= (rxConfig()->midrc + rcControlsConfig()->deadband3d_throttle)) {
...
#define THROTTLE_CLIPPING_FACTOR 0.33f
    ...
    if ((armingFlags & (ARMED))) {
        for (int i = 0; i < motorCount; i++) {
            ...
            if (feature(FEATURE_MOTOR_STOP) && (armingFlags & (ARMED))) {
                _Bool failsafeMotorStop = failsafeRequiresMotorStop();
                _Bool navMotorStop = !failsafeIsActive() && (stateFlags & (NAV_MOTOR_STOP_OR_IDLE));
                _Bool userMotorStop = !failsafeIsActive() && (rcData[THROTTLE] < rxConfig()->mincheck);
