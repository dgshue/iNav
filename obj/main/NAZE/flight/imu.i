...
static const float gyroScale = (3.14159265358979323846f / 180.0f);

static _Bool gpsHeadingInitialized = 0;
...
static void imuCheckAndResetOrientationQuaternion(const float ax, const float ay, const float az)
{
    ...
    const _Bool isNan = ((((sizeof(q0) == sizeof(float)) ? __fpclassifyf(q0) : __fpclassifyd(q0)) == 0) || (((sizeof(q1) == sizeof(float)) ? __fpclassifyf(q1) : __fpclassifyd(q1)) == 0) || (((sizeof(q2) == sizeof(float)) ? __fpclassifyf(q2) : __fpclassifyd(q2)) == 0) || (((sizeof(q3) == sizeof(float)) ? __fpclassifyf(q3) : __fpclassifyd(q3)) == 0));
    const _Bool isInf = ((((sizeof(q0) == sizeof(float)) ? __fpclassifyf(q0) : __fpclassifyd(q0)) == 1) || (((sizeof(q1) == sizeof(float)) ? __fpclassifyf(q1) : __fpclassifyd(q1)) == 1) || (((sizeof(q2) == sizeof(float)) ? __fpclassifyf(q2) : __fpclassifyd(q2)) == 1) || (((sizeof(q3) == sizeof(float)) ? __fpclassifyf(q3) : __fpclassifyd(q3)) == 1));
    const _Bool isZero = (( __extension__ ({ __typeof__(q0) _abs0 = (q0); _abs0 < 0 ? -_abs0 : _abs0; })) < 1e-3f && ( __extension__ ({ __typeof__(q1) _abs1 = (q1); _abs1 < 0 ? -_abs1 : _abs1; })) < 1e-3f && ( __extension__ ({ __typeof__(q2) _abs2 = (q2); _abs2 < 0 ? -_abs2 : _abs2; })) < 1e-3f && ( __extension__ ({ __typeof__(q3) _abs3 = (q3); _abs3 < 0 ? -_abs3 : _abs3; })) < 1e-3f);
    ...
    if (isNan || isZero || isInf) {
        ...
        if (feature(FEATURE_BLACKBOX)) {
            blackboxLogEvent(FLIGHT_LOG_EVENT_IMU_FAILURE, ((void *)0));
        }
...
static void imuMahonyAHRSupdate(float dt, float gx, float gy, float gz,
                                ...
                                _Bool useCOG, float courseOverGround)
{
    ...
    const float spin_rate_sq = ((gx)*(gx)) + ((gy)*(gy)) + ((gz)*(gz));
    ...
    if (useMag || useCOG) {
        ...
        if (imuRuntimeConfig.dcm_ki_mag > 0.0f) {
            ...
            if (spin_rate_sq < ((((20) * (3.14159265358979323846f / 180.0f)))*(((20) * (3.14159265358979323846f / 180.0f))))) {
                integralMagX += imuRuntimeConfig.dcm_ki_mag * ex * dt;
    ...
    if (useAcc) {
        ...
        if (imuRuntimeConfig.dcm_ki_acc > 0.0f) {
            ...
            if (spin_rate_sq < ((((20) * (3.14159265358979323846f / 180.0f)))*(((20) * (3.14159265358979323846f / 180.0f))))) {
                integralAccX += imuRuntimeConfig.dcm_ki_acc * ex * dt;
...
static void imuUpdateEulerAngles(void)
{
    ...
    attitude.values.yaw = (((-atan2_approx(rMat[1][0], rMat[0][0])) * 10.0f) / (3.14159265358979323846f / 180.0f)) + mag.magneticDeclination;
...
static _Bool imuCanUseAccelerometerForCorrection(void)
{
    int32_t axis;
    ...
    const int nearness = ( __extension__ ({ __typeof__(100 - (accMagnitudeSq * 100 / ((int32_t)acc.dev.acc_1G * acc.dev.acc_1G))) _abs4 = (100 - (accMagnitudeSq * 100 / ((int32_t)acc.dev.acc_1G * acc.dev.acc_1G))); _abs4 < 0 ? -_abs4 : _abs4; }));

    return (nearness > 25) ? 0 : 1;
}

static void imuCalculateEstimatedAttitude(float dT)
{

    const _Bool canUseMAG = sensors(SENSOR_MAG) && compassIsHealthy();




    const _Bool useAcc = imuCanUseAccelerometerForCorrection();
    ...
    if ((stateFlags & (FIXED_WING))) {
        _Bool canUseCOG = sensors(SENSOR_GPS) && (stateFlags & (GPS_FIX)) && gpsSol.numSat >= 6 && gpsSol.groundSpeed >= 300;

        if (canUseCOG) {
            if (gpsHeadingInitialized) {

                if (( __extension__ ({ __typeof__(gpsSol.groundCourse - attitude.values.yaw) _abs5 = (gpsSol.groundCourse - attitude.values.yaw); _abs5 < 0 ? -_abs5 : _abs5; })) < ((60) * 10) || !canUseMAG) {
...
# 563 "./src/main/flight/imu.c"
    for (axis = 0; axis < 3; axis++) {
        imuMeasuredAccelBF.A[axis] = acc.accADC[axis] * (980.665f / acc.dev.acc_1G);
    }

}
# 587 "./src/main/flight/imu.c"
void imuUpdateAccelerometer(void)
{
