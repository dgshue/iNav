...
static _Bool pidGainsUpdateRequired = 0;
int16_t axisPID[3];
...
void updatePIDCoefficients(void)
{
    ...
    const float tpaFactor = (stateFlags & (FIXED_WING)) ? calculateFixedWingTPAFactor() : calculateMultirotorTPAFactor();
    ...
    pidGainsUpdateRequired = 0;
}

static float calcHorizonRateMagnitude(void)
{

    const int32_t stickPosAil = ( __extension__ ({ __typeof__(getRcStickDeflection(FD_ROLL, rxConfig()->midrc)) _abs0 = (getRcStickDeflection(FD_ROLL, rxConfig()->midrc)); _abs0 < 0 ? -_abs0 : _abs0; }));
    const int32_t stickPosEle = ( __extension__ ({ __typeof__(getRcStickDeflection(FD_PITCH, rxConfig()->midrc)) _abs1 = (getRcStickDeflection(FD_PITCH, rxConfig()->midrc)); _abs1 < 0 ? -_abs1 : _abs1; }));
...
static void pidLevel(pidState_t *pidState, flight_dynamics_index_t axis, float horizonRateMagnitude)
{
    ...
    float angleRateTarget = constrainf(angleErrorDeg * (pidBank()->pid[PID_LEVEL].P / 6.56f), -currentControlRateProfile->rates[axis] * 10.0f, currentControlRateProfile->rates[axis] * 10.0f);
...
static void pidApplyFixedWingRateController(pidState_t *pidState, flight_dynamics_index_t axis)
{
    ...
    if ((stateFlags & (ANTI_WINDUP))) {
        ...
        pidState->errorGyroIfLimit = ( __extension__ ({ __typeof__(pidState->errorGyroIf) _abs4 = (pidState->errorGyroIf); _abs4 < 0 ? -_abs4 : _abs4; }));
    }
...
static void pidApplyMulticopterRateController(pidState_t *pidState, flight_dynamics_index_t axis)
{
    ...
    const float antiWindupScaler = constrainf(1.0f - (( __extension__ ({ __typeof__(pidState->rateTarget) _abs5 = (pidState->rateTarget); _abs5 < 0 ? -_abs5 : _abs5; })) / integratorThreshold), 0.0f, 1.0f);
...
static uint8_t getHeadingHoldState()
{
    ...
    if (( __extension__ ({ __typeof__(rcCommand[YAW]) _abs7 = (rcCommand[YAW]); _abs7 < 0 ? -_abs7 : _abs7; })) == 0 && (flightModeFlags & (HEADING_MODE))) {
        return HEADING_HOLD_ENABLED;
    } else {
...
# 658 "./src/main/flight/pid.c"
            ...
            targetRates.V.Z = ((coordinatedTurnRateEarthFrame) / (3.14159265358979323846f / 180.0f));
        }
        else {
