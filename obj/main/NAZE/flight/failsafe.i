...
const failsafeConfig_t pgResetTemplate_failsafeConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .failsafe_delay = 5, .failsafe_recovery_delay = 5, .failsafe_off_delay = 200, .failsafe_throttle = 1000, .failsafe_throttle_low_delay = 100, .failsafe_procedure = 0, .failsafe_fw_roll_angle = -200, .failsafe_fw_pitch_angle = 100, .failsafe_fw_yaw_rate = -45, .failsafe_stick_motion_threshold = 50, }
# 78 "./src/main/flight/failsafe.c"
 ;
...
static _Bool failsafeCheckStickMotion(void)
{
    if (failsafeConfig()->failsafe_stick_motion_threshold > 0) {
        uint32_t totalRcDelta = 0;

        totalRcDelta += ( __extension__ ({ __typeof__(rcData[ROLL] - rxConfig()->midrc) _abs0 = (rcData[ROLL] - rxConfig()->midrc); _abs0 < 0 ? -_abs0 : _abs0; }));
        totalRcDelta += ( __extension__ ({ __typeof__(rcData[PITCH] - rxConfig()->midrc) _abs1 = (rcData[PITCH] - rxConfig()->midrc); _abs1 < 0 ? -_abs1 : _abs1; }));
        totalRcDelta += ( __extension__ ({ __typeof__(rcData[YAW] - rxConfig()->midrc) _abs2 = (rcData[YAW] - rxConfig()->midrc); _abs2 < 0 ? -_abs2 : _abs2; }));
...
void failsafeUpdateState(void)
{
    ...
    const _Bool receivingRxData = failsafeIsReceivingRxData();
    const _Bool armed = (armingFlags & (ARMED));
    const _Bool sticksAreMoving = failsafeCheckStickMotion();
