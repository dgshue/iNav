...
static pwmOutputPort_t *motors[6];


static pwmOutputPort_t *servos[8];
# 64 "./src/main/drivers/pwm_output.c"
...
{
# 96 "./src/main/drivers/pwm_output.c"
    ...
    if (output & TIMER_OUTPUT_N_CHANNEL) {
        ...
        TIM_OCInitStructure.TIM_OCNPolarity = (output & TIMER_OUTPUT_INVERTED) ? ((uint16_t)0x0002) : ((uint16_t)0x0000);
        TIM_OCInitStructure.TIM_OCNIdleState = (output & TIMER_OUTPUT_INVERTED) ? ((uint16_t)0x0100) : ((uint16_t)0x0000);
    } else {
        TIM_OCInitStructure.TIM_OutputState = ((uint16_t)0x0001);
        TIM_OCInitStructure.TIM_OutputNState = ((uint16_t)0x0000);
        TIM_OCInitStructure.TIM_OCPolarity = (output & TIMER_OUTPUT_INVERTED) ? ((uint16_t)0x0000) : ((uint16_t)0x0002);
        TIM_OCInitStructure.TIM_OCIdleState = (output & TIMER_OUTPUT_INVERTED) ? ((uint16_t)0x0000) : ((uint16_t)0x0100);
    }
...
# 150 "./src/main/drivers/pwm_output.c"
...
static pwmOutputPort_t *pwmOutConfigMotor(const timerHardware_t *timerHardware, uint8_t mhz, uint16_t period, uint16_t value, _Bool enableOutput)
{
    ...
    if (enableOutput) {
        ...
        IOConfigGPIO(io, ((GPIO_Mode_AF_PP) | (GPIO_Speed_2MHz)));
        ...
        IOConfigGPIO(io, ((GPIO_Mode_Out_OD) | (GPIO_Speed_2MHz)));
        IOLo(io);
    }
...
{
    *motors[index]->ccr = lrintf((float)(value * 8/8.0f));
}

static void pwmWriteOneShot42(uint8_t index, uint16_t value)
{
    *motors[index]->ccr = lrintf((float)(value * 24/24.0f));
}

static void pwmWriteMultiShot(uint8_t index, uint16_t value)
{
    *motors[index]->ccr = lrintf(((float)(value-1000) * (72 * 20 / 1000.0f)) + (72 * 5));
}


void pwmWriteMotor(uint8_t index, uint16_t value)
{
    if (motors[index] && index < 6 && pwmMotorsEnabled) {
...
void pwmMotorConfig(const timerHardware_t *timerHardware, uint8_t motorIndex, uint16_t motorPwmRate, uint16_t idlePulse, motorPwmProtocolTypes_e proto, _Bool enableOutput)
{
...
void pwmServoConfig(const timerHardware_t *timerHardware, uint8_t servoIndex, uint16_t servoPwmRate, uint16_t servoCenterPulse, _Bool enableOutput)
{
    servos[servoIndex] = pwmOutConfigMotor(timerHardware, 1, 1000000 / servoPwmRate, servoCenterPulse, enableOutput);
}

void pwmWriteServo(uint8_t index, uint16_t value)
{
# 310 "./src/main/drivers/pwm_output.c"
