...
_Bool CheckGPIOPin(ioTag_t tag, GPIO_TypeDef *gpio, uint16_t pin)
{
    return IO_GPIO(IOGetByTag(tag)) == gpio && IO_Pin(IOGetByTag(tag)) == pin;
}

_Bool CheckGPIOPinSource(ioTag_t tag, GPIO_TypeDef *gpio, uint16_t pin)
{
...
pwmIOConfiguration_t *pwmInit(drv_pwm_config_t *init)
{
    ...
    for (int timerIndex = 0; timerIndex < 14; timerIndex++) {
        const timerHardware_t *timerHardwarePtr = &timerHardware[timerIndex];
        int type = MAP_TO_NONE;
# 82 "./src/main/drivers/pwm_mapping.c"
        if (init->useUART2 && (timerHardwarePtr->tag == ((((0) + 1) << 4) | (2)) || timerHardwarePtr->tag == ((((0) + 1) << 4) | (3)))) {
...
# 104 "./src/main/drivers/pwm_mapping.c"
        const timerHardware_t *ss1TimerHardware = timerGetByTag(((((0) + 1) << 4) | (6)), TIM_USE_ANY);
        if (init->useSoftSerial && ss1TimerHardware != ((void *)0) && ss1TimerHardware->tim == timerHardwarePtr->tim) {
            addBootlogEvent6(BOOT_EVENT_TIMER_CH_SKIPPED, BOOT_EVENT_FLAGS_WARNING, timerIndex, pwmIOConfiguration.motorCount, pwmIOConfiguration.servoCount, 3);
            continue;
        }
        ...
        const timerHardware_t *ss2TimerHardware = timerGetByTag(((((1) + 1) << 4) | (0)), TIM_USE_ANY);
        if (init->useSoftSerial && ss2TimerHardware != ((void *)0) && ss2TimerHardware->tim == timerHardwarePtr->tim) {
            addBootlogEvent6(BOOT_EVENT_TIMER_CH_SKIPPED, BOOT_EVENT_FLAGS_WARNING, timerIndex, pwmIOConfiguration.motorCount, pwmIOConfiguration.servoCount, 3);
            continue;
        }
# 169 "./src/main/drivers/pwm_mapping.c"
        if (init->useSerialRx) {
            type = MAP_TO_NONE;
        }
        ...
        if (type == MAP_TO_NONE)
            continue;
# 237 "./src/main/drivers/pwm_mapping.c"
        if (type == MAP_TO_PPM_INPUT) {
            ...
            if (pwmIOConfiguration.motorCount >= 6) {
