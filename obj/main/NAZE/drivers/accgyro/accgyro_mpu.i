...
static void mpu6050FindRevision(gyroDev_t *gyro)
{
    ...
    ack = gyro->mpuConfiguration.readFn(&gyro->bus, 0x06, 6, readBuffer);
    ...
    if (revision) {
        ...
        ack = gyro->mpuConfiguration.readFn(&gyro->bus, 0x0C, 1, &productId);
...
static void mpuIntExtiHandler(extiCallbackRec_t *cb)
{
    ...
    gyroDev_t *gyro = ( __extension__ ({ const typeof( ((gyroDev_t *)0)->exti ) *__mptr = (cb); (gyroDev_t *)( (char *)__mptr - __builtin_offsetof (gyroDev_t, exti) );}));
    gyro->dataReady = 1;
    if (gyro->updateFn) {
        gyro->updateFn(gyro);
    }
...
# 150 "./src/main/drivers/accgyro/accgyro_mpu.c"
    IOInit(mpuIntIO, OWNER_MPU, RESOURCE_EXTI, 0);
    IOConfigGPIO(mpuIntIO, ((GPIO_Mode_IN_FLOATING) | (GPIO_Speed_2MHz)));
...
{
    (void)(bus);
    const _Bool ack = i2cRead((I2CDEV_2), 0x68, reg, length, data);
    return ack;
}
...
{
    (void)(bus);
    const _Bool ack = i2cWrite((I2CDEV_2), 0x68, reg, data);
    return ack;
}

_Bool mpuAccRead(accDev_t *acc)
{
    uint8_t data[6];

    const _Bool ack = acc->mpuConfiguration.readFn(&acc->bus, 0x3B, 6, data);
    if (!ack) {
        return 0;
    }
...
{
    for ( uint8_t __basepri_save __attribute__((__cleanup__(__basepriRestoreMem))) = __get_BASEPRI(), __ToDo = __basepriSetMemRetVal((((((0x0f)<<(4-(7-(((uint32_t)0x500)>>8))))|((0x0f)&(0x0f>>(7-(((uint32_t)0x500)>>8)))))<<4)&0xf0)); __ToDo ; __ToDo = 0 ) {
        gyro->updateFn = updateFn;
    }
}

_Bool mpuGyroRead(gyroDev_t *gyro)
{
    uint8_t data[6];

    const _Bool ack = gyro->mpuConfiguration.readFn(&gyro->bus, gyro->mpuConfiguration.gyroReadXRegister, 6, data);
    if (!ack) {
        return 0;
    }
...
_Bool mpuCheckDataReady(gyroDev_t* gyro)
{
    _Bool ret;
    if (gyro->dataReady) {
        ret = 1;
...
static _Bool detectSPISensorsAndUpdateDetectionResult(gyroDev_t *gyro)
{
    ...
    (void)(sensor);
# 305 "./src/main/drivers/accgyro/accgyro_mpu.c"
    return 0;
}
...
void mpuDetect(gyroDev_t *gyro)
{
    ...
    _Bool ack = mpuReadRegisterI2C(&gyro->bus, 0x75, 1, &sig);
    ...
    gyro->mpuConfiguration.gyroReadXRegister = 0x43;


    uint8_t inquiryResult;
    ack = mpuReadRegisterI2C(&gyro->bus, 0x00, 1, &inquiryResult);
    inquiryResult &= 0x7E;
    if (ack && inquiryResult == (0x68)) {
        gyro->mpuDetectionResult.sensor = MPU_3050;
        gyro->mpuConfiguration.gyroReadXRegister = 0x1D;
        return;
    }
