...
# 22 "./src/main/drivers/serial_softserial.c" 2
...
{
    IOInit(IOGetByTag(pin), OWNER_SOFTSERIAL, RESOURCE_UART_RX, (portIndex + 1));

    IOConfigGPIO(IOGetByTag(pin), ((GPIO_Mode_IPU) | (GPIO_Speed_2MHz)));
...
{
    IOInit(IOGetByTag(pin), OWNER_SOFTSERIAL, RESOURCE_UART_TX, (portIndex + 1));
    IOConfigGPIO(IOGetByTag(pin), ((GPIO_Mode_Out_PP) | (GPIO_Speed_2MHz)));
}
...
serialPort_t *openSoftSerial(softSerialPortIndex_e portIndex, serialReceiveCallbackPtr rxCallback, uint32_t baud, portOptions_t options)
{
    ...
    if (portIndex == SOFTSERIAL1) {
        softSerial->rxTimerHardware = timerGetByTag(((((0) + 1) << 4) | (6)), TIM_USE_ANY);;
        softSerial->txTimerHardware = timerGetByTag(((((0) + 1) << 4) | (7)), TIM_USE_ANY);;
    }



    if (portIndex == SOFTSERIAL2) {
        softSerial->rxTimerHardware = timerGetByTag(((((1) + 1) << 4) | (0)), TIM_USE_ANY);;
        softSerial->txTimerHardware = timerGetByTag(((((1) + 1) << 4) | (1)), TIM_USE_ANY);;
    }
...
serialPort_t *softSerialLoopbackPort(void)
{
    ...
    if (!loopbackPort->vTable) {
        loopbackPort = openSoftSerial(0, ((void *)0), 19200, SERIAL_NOT_INVERTED);
    }
    return loopbackPort;
}
