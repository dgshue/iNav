...
# 8 "./src/main/drivers/io_pca9685.c" 2
...
# 11 "./src/main/drivers/io_pca9685.c" 2
...
uint16_t currentOutputState[16] = {0};
uint16_t temporaryOutputState[16] = {0};

void pca9685setPWMOn(uint8_t servoIndex, uint16_t on) {
    if (servoIndex < 16) {
        i2cWrite((I2CDEV_2), 0x40, 0x06 + (servoIndex * 4), on);
        i2cWrite((I2CDEV_2), 0x40, 0x07 + (servoIndex * 4), on>>8);
    }
}

void pca9685setPWMOff(uint8_t servoIndex, uint16_t off) {
    if (servoIndex < 16) {
        i2cWrite((I2CDEV_2), 0x40, 0x08 + (servoIndex * 4), off);
        i2cWrite((I2CDEV_2), 0x40, 0x09 + (servoIndex * 4), off>>8);
    }
}
# 58 "./src/main/drivers/io_pca9685.c"
...
void pca9685setPWMFreq(uint16_t freq) {
  ...
  i2cWrite((I2CDEV_2), 0x40, 0x00, 16);
  delay(1);
  i2cWrite((I2CDEV_2), 0x40, 0xFE, (uint8_t) prescaleval);
  delay(1);
  i2cWrite((I2CDEV_2), 0x40, 0x00, 128);
}
...
_Bool pca9685Initialize(void) {
    ...
    ack = i2cRead((I2CDEV_2), 0x40, 0x00, 1, &sig);
    ...
    if (!ack) {
        ...
        i2cWrite((I2CDEV_2), 0x40, 0x00, 0x0);
