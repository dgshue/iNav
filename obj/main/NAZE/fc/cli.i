...
# 1 "./src/main/fc/settings_generated.h" 1
...
typedef uint32_t clivalue_max_t;
#define CLIVALUE_MIN_MAX_INDEX_BYTES 2
enum {
 TABLE_ACC_HARDWARE,
 TABLE_ALIGNMENT,
 TABLE_AUX_OPERATOR,
...
extern const char *table_acc_hardware[];
extern const char *table_alignment[];
extern const char *table_aux_operator[];
extern const char *table_baro_hardware[];
extern const char *table_blackbox_device[];
extern const char *table_current_sensor[];
extern const char *table_debug_modes[];
extern const char *table_failsafe_procedure[];
extern const char *table_frsky_unit[];
extern const char *table_gimbal_mode[];
extern const char *table_gps_dyn_model[];
extern const char *table_gps_provider[];
extern const char *table_gps_sbas_mode[];
...
extern const char *table_off_on[];
extern const char *table_reset_altitude[];
extern const char *table_serial_rx[];
# 9 "./src/main/fc/settings.h" 2
...
_Bool clivalue_name_contains(const clivalue_t *val, char *buf, const char *cmdline);
_Bool clivalue_name_exact_match(const clivalue_t *val, char *buf, const char *cmdline, uint8_t var_name_length);
...
_Bool rxSpiInit(const rxConfig_t *rxConfig, rxRuntimeConfig_t *rxRuntimeConfig);
...
typedef enum {
    ...
    TASK_BARO,
# 87 "./src/main/scheduler/scheduler.h"
    TASK_TELEMETRY,
# 108 "./src/main/scheduler/scheduler.h"
    TASK_COUNT,
...
typedef struct {
    ...
    const char * taskName;
    _Bool (*checkFunc)(timeUs_t currentTimeUs, timeDelta_t currentDeltaTimeUs);
...
_Bool isHardwareHealthy(void);
# 108 "./src/main/fc/cli.c" 2
...
static const char * const featureNames[] = {
    ...
    "SUPEREXPO", "VTX", "RX_SPI", "SOFTSPI", "PWM_SERVO_DRIVER", "PWM_OUTPUT_ENABLE", "OSD", ((void *)0)
};
...
static const char * const sensorTypeNames[] = {
    "GYRO", "ACC", "BARO", "MAG", "RANGEFINDER", "PITOT", "GPS", "GPS+MAG", ((void *)0)
};
...
static _Bool cliDumpPrintLinef(uint8_t dumpMask, _Bool equalsDefault, const char *format, ...)
{
...
static _Bool cliDefaultPrintLinef(uint8_t dumpMask, _Bool equalsDefault, const char *format, ...)
{
...
static _Bool valuePtrEqualsDefault(uint8_t type, const void *ptr, const void *ptrDefault)
{
    _Bool result = 0;
    switch (type & (0x0F)) {
    case VAR_UINT8:
...
static void dumpPgValue(const clivalue_t *value, uint8_t dumpMask)
{
    ...
    const _Bool equalsDefault = valuePtrEqualsDefault(value->type, pg->copy + valueOffset, pg->address + valueOffset);
...
{
    return (string == ((void *)0) || *string == '\0') ? 1 : 0;
}
# 615 "./src/main/fc/cli.c"
...
static void cliAux(char *cmdline)
{
    ...
    if (isEmpty(cmdline)) {
        printAux(DUMP_MASTER, modeActivationConditions(0), ((void *)0));
    } else {
        ptr = cmdline;
        i = fastA2I(ptr++);
        if (i < 20) {
...
{
    if (isEmpty(cmdline)) {
        printSerial(DUMP_MASTER, serialConfig(), ((void *)0));
        return;
    }
...
static void cliAdjustmentRange(char *cmdline)
{
    ...
    if (isEmpty(cmdline)) {
        printAdjustmentRange(DUMP_MASTER, adjustmentRanges(0), ((void *)0));
    } else {
        ptr = cmdline;
        i = fastA2I(ptr++);
        if (i < 12) {
...
static void printMotorMix(uint8_t dumpMask, const motorMixer_t *customMotorMixer, const motorMixer_t *defaultCustomMotorMixer)
{
    ...
    for (uint32_t i = 0; i < 6; i++) {
        ...
        if (defaultCustomMotorMixer) {
            ...
            const _Bool equalsDefault = thr == thrDefault && roll == rollDefault && pitch == pitchDefault && yaw == yawDefault;
...
static void cliMotorMix(char *cmdline)
{
    ...
    if (isEmpty(cmdline)) {
        printMotorMix(DUMP_MASTER, customMotorMixer(0), ((void *)0));
        ...
        if (i < 6) {
            ...
            cliShowArgumentRangeError("index", 0, 6 - 1);
        }
    }
}
...
static void cliRxRange(char *cmdline)
{
    ...
    if (isEmpty(cmdline)) {
        printRxRange(DUMP_MASTER, rxChannelRangeConfigs(0), ((void *)0));
...
static void printServo(uint8_t dumpMask, const servoParam_t *servoParam, const servoParam_t *defaultServoParam)
{
    ...
    if (defaultServoParam) {
        for (uint32_t i = 0; i < 8; i++) {
            const servoParam_t *servoConf = &servoParam[i];
            const servoParam_t *servoConfDefault = &defaultServoParam[i];
            _Bool equalsDefault = servoConf->reversedSources == servoConfDefault->reversedSources;
...
static void cliServo(char *cmdline)
{
    ...
    if (isEmpty(cmdline)) {
        printServo(DUMP_MASTER, servoParams(0), ((void *)0));
    } else {
...
static void cliServoMix(char *cmdline)
{
    ...
    if (len == 0) {
        printServoMix(DUMP_MASTER, customServoMixers(0), ((void *)0));
    } else if (sl_strncasecmp(cmdline, "reset", 5) == 0) {

        pgResetCopy(customServoMixersMutable(0), 21);
        for (uint32_t i = 0; i < 8; i++) {
...
{
    if (!sl_strncasecmp(cmdLine, command, strlen(command))
        && !sl_isalnum((unsigned)cmdLine[strlen(command)])) {
...
static void cliRebootEx(_Bool bootLoader)
{
    cliPrint("\r\nRebooting");
    bufWriterFlush(cliWriter);
...
{
    _Bool equalsDefault = strlen(sConfig->name) == 0;
...
static void cliName(char *cmdline)
{
    ...
    if (len > 0) {
        memset(systemConfigMutable()->name, 0, (sizeof(systemConfigMutable()->name) / sizeof((systemConfigMutable()->name)[0])));
        if (strncmp(cmdline, emptyName, len)) {
            strncpy(systemConfigMutable()->name, cmdline, ( __extension__ ({ __typeof__(len) _left0 = (len); __typeof__(16) _right1 = (16); _left0 < _right1 ? _left0 : _right1; })));
        }
    }
    printName(DUMP_MASTER, systemConfig());
}
# 2141 "./src/main/fc/cli.c"
                ...
                case MODE_DIRECT: {
                        if (*eqptr != 0 && strspn(eqptr, "0123456789.+-") == strlen(eqptr)) {
                            ...
                            if (valuef >= (float)clivalue_get_min(val) && valuef <= (float)clivalue_get_max(val)) {
                                ...
                                if ((cliValueTable[i].type & (0x0F)) == VAR_FLOAT)
                                    tmp.float_value = valuef;
                                else if ((cliValueTable[i].type & (0x0F)) == VAR_UINT32)
...
static void cliStatus(char *cmdline)
{
    ...
    for (int i = 0; i < SENSOR_INDEX_COUNT; i++) {
        ...
        if ((detectedSensorsMask & mask) && (mask & (SENSOR_GYRO | SENSOR_ACC | SENSOR_BARO | SENSOR_MAG | SENSOR_RANGEFINDER | SENSOR_PITOT))) {
...
static void restoreConfigs(void)
{
    for (const pgRegistry_t *(pg) = __pg_registry_start; (pg) < __pg_registry_end; pg++) {
        if (pgIsProfile(pg)) {
            memcpy(pg->address, pg->copy, pgSize(pg) * 3);
        } else {
...
static void printConfig(const char *cmdline, _Bool doDiff)
{
    ...
    if ((dumpMask & DUMP_MASTER) || (dumpMask & DUMP_ALL)) {
        ...
        ;
        const _Bool equalsDefault = mixerConfig_Copy.mixerMode == mixerConfig()->mixerMode;
...
{
    printConfig(cmdline, 0);
}

static void cliDiff(char *cmdline)
{
    printConfig(cmdline, 1);
}
...
# 2680 "./src/main/fc/cli.c"
    ...
    { "gpspassthrough", cliGpsPassthrough },
    ...
    while (serialRxBytesWaiting(cliPort)) {
        ...
        if (c == '\t' || c == '?') {
            ...
            if (bufferIndex > 0) {
                ...
                const clicmd_t *cmd;
                for (cmd = cmdTable; cmd < cmdTable + (sizeof(cmdTable) / sizeof((cmdTable)[0])); cmd++) {
                    if (!sl_strncasecmp(cliBuffer, cmd->name, strlen(cmd->name))
                       && !sl_isalnum((unsigned)cliBuffer[strlen(cmd->name)]))
                        break;
                }
...
void cliEnter(serialPort_t *serialPort)
{
    ...
    cliPrompt();
    (armingFlags |= (ARMING_DISABLED_CLI));
}
