...
_Bool bitArrayGet(const void *array, unsigned bit);
...
_Bool max7456DmaInProgress(void);
# 41 "./src/main/fc/fc_msp.c" 2
...
# 324 "./src/main/msp/msp_protocol.h" 2
...
# 100 "./src/main/fc/fc_msp.c" 2


extern timeDelta_t cycleTime;
extern uint16_t rssi;

static const char * const flightControllerIdentifier = "INAV";
...
static void initActiveBoxIds(void)
{
    ...
    if ((feature(FEATURE_GPS) && sensors(SENSOR_MAG) && sensors(SENSOR_ACC)) || ((stateFlags & (FIXED_WING)) && sensors(SENSOR_ACC) && feature(FEATURE_GPS))) {
...
static void packBoxModeFlags(boxBitmask_t * mspBoxModeFlags)
{
    ...
    do { if ((((flightModeFlags & (PASSTHRU_MODE)) == 0 ? 0 : 1) == 0 ? 0 : 1)) { activeBoxes[BOXPASSTHRU] = 1; } } while(0);
    ...
    do { if (((IS_RC_MODE_ACTIVE(BOXLLIGHTS) == 0 ? 0 : 1) == 0 ? 0 : 1)) { activeBoxes[BOXLLIGHTS] = 1; } } while(0);
    ...
    do { if (((IS_RC_MODE_ACTIVE(BOXSURFACE) == 0 ? 0 : 1) == 0 ? 0 : 1)) { activeBoxes[BOXSURFACE] = 1; } } while(0);
...
static _Bool mspFcProcessOutCommand(uint8_t cmdMSP, sbuf_t *dst, mspPostProcessFnPtr *mspPostProcessFn)
{
    switch (cmdMSP) {
    case 1:
        sbufWriteU8(dst, 0);
    ...
    case 5:
        ...
        sbufWriteData(dst, shortGitRevision, 8);
        break;
    ...
    case 100:
        ...
        sbufWriteU8(dst, 0);
        sbufWriteU32(dst, ((uint32_t)1 << 31) | ((uint32_t)1 << 2) | ((uint32_t)1 << 3) | ((uint32_t)1 << 4) | ((uint32_t)1 << 5));
        break;
# 585 "./src/main/fc/fc_msp.c"
    case 151:
    ...
    case 104:
        for (unsigned i = 0; i < 8; i++) {
            sbufWriteU16(dst, i < 6 ? motor[i] : 0);
        }
        break;
    ...
    case 110:
        ...
        sbufWriteU16(dst, (uint16_t)constrain(mAhDrawn, 0, 0xFFFF));
        sbufWriteU16(dst, rssi);
        if (batteryConfig()->multiwiiCurrentMeterOutput) {
            sbufWriteU16(dst, (uint16_t)constrain(amperage * 10, 0, 0xFFFF));
        } else
            sbufWriteU16(dst, (int16_t)constrain(amperage, -0x8000, 0x7FFF));
        break;
...
# 1267 "./src/main/fc/fc_msp.c"
    ...
    case 16:
        ...
        sbufWriteU16(dst, positionEstimationConfig()->w_z_baro_p * 100);
        sbufWriteU16(dst, positionEstimationConfig()->w_z_gps_p * 100);
        sbufWriteU16(dst, positionEstimationConfig()->w_z_gps_v * 100);
        sbufWriteU16(dst, positionEstimationConfig()->w_xy_gps_p * 100);
        sbufWriteU16(dst, positionEstimationConfig()->w_xy_gps_v * 100);
        sbufWriteU8(dst, gpsConfigMutable()->gpsMinSats);
        sbufWriteU8(dst, positionEstimationConfig()->use_gps_velned);
# 1297 "./src/main/fc/fc_msp.c"
        break;
    ...
    case 20:
        ...
        sbufWriteU8(dst, 0);
        sbufWriteU8(dst, 30);
        sbufWriteU8(dst, isWaypointListValid());
        sbufWriteU8(dst, getWaypointCount());
...
{
    const uint8_t msp_wp_no = sbufReadU8(src);
    navWaypoint_t msp_wp;
    getWaypoint(msp_wp_no, &msp_wp);
    sbufWriteU8(dst, msp_wp_no);
    sbufWriteU8(dst, msp_wp.action);
    sbufWriteU32(dst, msp_wp.lat);
    sbufWriteU32(dst, msp_wp.lon);
    sbufWriteU32(dst, msp_wp.alt);
    sbufWriteU16(dst, msp_wp.p1);
    sbufWriteU16(dst, msp_wp.p2);
    sbufWriteU16(dst, msp_wp.p3);
    sbufWriteU8(dst, msp_wp.flag);
}
    ...
    case 204:
        if (dataSize >= 10) {
            ...
            for (int i = 0; i < 3; i++) {
                rate = sbufReadU8(src);
                if (i == FD_YAW) {
                    ((controlRateConfig_t*)currentControlRateProfile)->rates[i] = constrain(rate, 2, 180);
                }
                else {
                    ((controlRateConfig_t*)currentControlRateProfile)->rates[i] = constrain(rate, 6, 180);
                }
            }
            rate = sbufReadU8(src);
            ((controlRateConfig_t*)currentControlRateProfile)->dynThrPID = ( __extension__ ({ __typeof__(rate) _left0 = (rate); __typeof__(100) _right1 = (100); _left0 < _right1 ? _left0 : _right1; }));
    ...
    case 209:
        {
            const uint8_t msp_wp_no = sbufReadU8(src);
            navWaypoint_t msp_wp;
            msp_wp.action = sbufReadU8(src);
            msp_wp.lat = sbufReadU32(src);
            msp_wp.lon = sbufReadU32(src);
            msp_wp.alt = sbufReadU32(src);
            msp_wp.p1 = sbufReadU16(src);
            msp_wp.p2 = sbufReadU16(src);
            msp_wp.p3 = sbufReadU16(src);
            msp_wp.flag = sbufReadU8(src);
