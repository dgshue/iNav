...
static void updateArmingStatus(void)
{
    if ((armingFlags & (ARMED))) {
        ...
        static _Bool calibratingFinishedBeep = 0;
        if (isCalibrating()) {
            (armingFlags |= (ARMING_DISABLED_SENSORS_CALIBRATING));
            calibratingFinishedBeep = 0;
        }
        else {
            (armingFlags &= ~(ARMING_DISABLED_SENSORS_CALIBRATING));

            if (!calibratingFinishedBeep) {
                calibratingFinishedBeep = 1;
...
void processRx(timeUs_t currentTimeUs)
{
    ...
    if (!cliMode) {
        ...
        processRcAdjustments(((controlRateConfig_t*)(currentControlRateProfile)));
    }
...
# 550 "./src/main/fc/fc_core.c"
    ...
    if ((flightModeFlags & (PASSTHRU_MODE)) || !(armingFlags & (ARMED))) {
...
void taskGyro(timeUs_t currentTimeUs) {
    ...
    if (gyroConfig()->gyroSync) {
        while (1) {
            if (gyroSyncCheckUpdate() || ((currentDeltaTime + cmpTimeUs(micros(), currentTimeUs)) >= (getGyroUpdateRate() + 100))) {
                break;
            }
        }
    }
