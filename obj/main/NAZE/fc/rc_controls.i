...
const armingConfig_t pgResetTemplate_armingConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .disarm_kill_switch = 1, .auto_disarm_delay = 5 }
...
rollPitchStatus_e calculateRollPitchCenterStatus(void)
{
    if (((rcData[PITCH] < (rxConfig()->midrc + 25)) && (rcData[PITCH] > (rxConfig()->midrc -25)))
            && ((rcData[ROLL] < (rxConfig()->midrc + 25)) && (rcData[ROLL] > (rxConfig()->midrc -25))))
        return CENTERED;
...
void processRcStickPositions(throttleStatus_e throttleStatus, _Bool disarm_kill_switch, _Bool fixed_wing_auto_arm)
{
   ...
   if (isUsingSticksForArming()) {
        ...
        if (rcSticks == THR_LO + YAW_LO + PIT_CE + ROL_CE) {
            ...
            if ((stateFlags & (FIXED_WING)) && feature(FEATURE_MOTOR_STOP) && fixed_wing_auto_arm) {
                return;
            }
...
int32_t getRcStickDeflection(int32_t axis, uint16_t midrc) {
    return ( __extension__ ({ __typeof__(( __extension__ ({ __typeof__(rcData[axis] - midrc) _abs2 = (rcData[axis] - midrc); _abs2 < 0 ? -_abs2 : _abs2; }))) _left3 = (( __extension__ ({ __typeof__(rcData[axis] - midrc) _abs2 = (rcData[axis] - midrc); _abs2 < 0 ? -_abs2 : _abs2; }))); __typeof__(500) _right4 = (500); _left3 < _right4 ? _left3 : _right4; }));
}
