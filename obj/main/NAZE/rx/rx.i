...
_Bool jetiExBusInit(const rxConfig_t *rxConfig, rxRuntimeConfig_t *rxRuntimeConfig);
...
#define CRSF_PAYLOAD_SIZE_MAX 32
...
struct rxConfig_s;
struct rxRuntimeConfig_s;
_Bool crsfRxInit(const struct rxConfig_s *initialRxConfig, struct rxRuntimeConfig_s *rxRuntimeConfig);
_Bool crsfRxIsActive(void);
...
const rxConfig_t pgResetTemplate_rxConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .halfDuplex = 0, .serialrx_provider = 0, .rx_spi_protocol = 0, .spektrum_sat_bind = 0, .sbus_inversion = 1, .midrc = 1500, .mincheck = 1100, .maxcheck = 1900, .rx_min_usec = 885, .rx_max_usec = 2115, .rssi_channel = 0, .rssi_scale = 30, .rssiInvert = 0, .rcSmoothing = 1, }
# 127 "./src/main/rx/rx.c"
 ;
...
_Bool rxUpdateCheck(timeUs_t currentTimeUs, timeDelta_t currentDeltaTime)
{
    ...
    return rxDataReceived || ((int32_t)(currentTimeUs - rxUpdateAt) >= 0);
}

static uint16_t calculateNonDataDrivenChannel(uint8_t chan, uint16_t sample)
{
    static int16_t rcSamples[16][3];
    static int16_t rcDataMean[16];
...
{
    const _Bool rxIsDataDriven = isRxDataDriven();
...
static void updateRSSIADC(timeUs_t currentTimeUs)
{
    ...
    rssiUpdateAtUs = currentTimeUs + (1000000 / 50);
