...
# 21 "./src/main/drivers/barometer/barometer_bmp085.h" 2
...
_Bool bmp085Detect(const bmp085Config_t *config, baroDev_t *baro);
...
_Bool bmp280Detect(baroDev_t *baro);
...
struct baroDev_s;
_Bool fakeBaroDetect(struct baroDev_s *baro);
...
# 49 "./src/main/sensors/barometer.c" 2
...
void baroStartCalibration(void)
{
    ...
    baroCalibrationFinished = 0;
}

#define PRESSURE_SAMPLES_MEDIAN 3
# 207 "./src/main/sensors/barometer.c"
...
static void performBaroCalibrationCycle(void)
{
    ...
    if (( __extension__ ({ __typeof__(baroGroundPressureError) _abs1 = (baroGroundPressureError); _abs1 < 0 ? -_abs1 : _abs1; })) < (baroGroundPressure * 0.00005f)) {
        if ((millis() - baroCalibrationTimeout) > 250) {
            baroGroundAltitude = pressureToAltitude(baroGroundPressure);
            baroCalibrationFinished = 1;
        }
    }
    else {
        baroCalibrationTimeout = millis();
