...
extern const batteryConfig_t pgResetTemplate_batteryConfig; batteryConfig_t batteryConfig_System; batteryConfig_t batteryConfig_Copy; extern const pgRegistry_t batteryConfig_Registry; const pgRegistry_t batteryConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 11 | (0 << 12), .size = sizeof(batteryConfig_t) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&batteryConfig_System, .copy = (uint8_t*)&batteryConfig_Copy, .ptr = 0, .reset = {.ptr = (void*)&pgResetTemplate_batteryConfig}, };

const batteryConfig_t pgResetTemplate_batteryConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .vbatscale = 110, .vbatresdivval = 10, .vbatresdivmultiplier = 1, .vbatmaxcellvoltage = 43, .vbatmincellvoltage = 33, .vbatwarningcellvoltage = 35, .currentMeterOffset = 0, .currentMeterScale = 400, .batteryCapacity = 0, .currentMeterType = CURRENT_SENSOR_ADC }
# 75 "./src/main/sensors/battery.c"
 ;
...
static void updateBatteryVoltage(uint32_t vbatTimeDelta)
{
    ...
    vbatSample = pt1FilterApply4(&vbatFilterState, vbatSample, 1, vbatTimeDelta * 1e-6f);
