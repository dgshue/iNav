...
_Bool adxl345Detect(accDev_t *acc, drv_adxl345_config_t *init);
...
_Bool bma280Detect(accDev_t *acc);
...
_Bool fakeAccDetect(accDev_t *acc);
void fakeAccSet(int16_t x, int16_t y, int16_t z);

_Bool fakeGyroDetect(gyroDev_t *gyro);
...
_Bool l3g4200dDetect(gyroDev_t *gyro);
...
_Bool l3gd20Detect(gyroDev_t *gyro);
...
# 72 "./src/main/drivers/accgyro/accgyro_lsm303dlhc.h"
...
# 177 "./src/main/drivers/accgyro/accgyro_lsm303dlhc.h"
...
# 406 "./src/main/drivers/accgyro/accgyro_lsm303dlhc.h"
...
_Bool lsm303dlhcAccDetect(accDev_t *acc);
...
_Bool mma8452Detect(accDev_t *acc);
...
_Bool mpu3050Detect(gyroDev_t *gyro);
...
_Bool mpu6050AccDetect(accDev_t *acc);
_Bool mpu6050GyroDetect(gyroDev_t *gyro);
...
_Bool mpu6500AccDetect(accDev_t *acc);
_Bool mpu6500GyroDetect(gyroDev_t *gyro);
...
_Bool mpu6000SpiDetect(const busDevice_t *bus);

_Bool mpu6000SpiAccDetect(accDev_t *acc);
_Bool mpu6000SpiGyroDetect(gyroDev_t *gyro);

_Bool mpu6000SpiWriteRegister(const busDevice_t *bus, uint8_t reg, uint8_t data);
_Bool mpu6000SpiReadRegister(const busDevice_t *bus, uint8_t reg, uint8_t length, uint8_t *data);
...
_Bool mpu6500SpiAccDetect(accDev_t *acc);
_Bool mpu6500SpiGyroDetect(gyroDev_t *gyro);

_Bool mpu6500SpiWriteRegister(const busDevice_t *bus, uint8_t reg, uint8_t data);
_Bool mpu6500SpiReadRegister(const busDevice_t *bus, uint8_t reg, uint8_t length, uint8_t *data);
...
# 19 "./src/main/drivers/accgyro/accgyro_spi_mpu9250.h"
#define GYRO_SCALE_FACTOR 0.00053292f
...
_Bool mpu9250SpiDetect(const busDevice_t *bus);

_Bool mpu9250SpiAccDetect(accDev_t *acc);
_Bool mpu9250SpiGyroDetect(gyroDev_t *gyro);

_Bool mpu9250SpiWriteRegister(const busDevice_t *bus, uint8_t reg, uint8_t data);
_Bool verifympu9250SpiWriteRegister(const busDevice_t *bus, uint8_t reg, uint8_t data);
_Bool mpu9250SpiReadRegister(const busDevice_t *bus, uint8_t reg, uint8_t length, uint8_t *data);
_Bool mpu9250SpiSlowReadRegister(const busDevice_t *bus, uint8_t reg, uint8_t length, uint8_t *data);
...
static _Bool accDetect(accDev_t *dev, accelerationSensor_e accHardwareToUse)
{
    ...
    case ACC_AUTODETECT:
        ;
# 154 "./src/main/sensors/acceleration.c"
    case ACC_MPU6050:
...
# 219 "./src/main/sensors/acceleration.c"
    case ACC_MPU6500:
...
# 264 "./src/main/sensors/acceleration.c"
    default:
    case ACC_NONE:
...
_Bool accInit(uint32_t targetLooptime)
{
...
static _Bool isOnFinalAccelerationCalibrationCycle(void)
{
    return calibratingA == 1;
}

static _Bool isOnFirstAccelerationCalibrationCycle(void)
{
    return calibratingA == 400;
}

static sensorCalibrationState_t calState;
static _Bool calibratedAxis[6];
...
_Bool accGetCalibrationAxisStatus(int axis)
{
    if (accIsCalibrationComplete()) {
        if ((stateFlags & (ACCELEROMETER_CALIBRATED))) {
            return 1;
        }
        else {
...
static void performAcclerationCalibration(void)
{
    ...
    if (calibratedAxisCount == 6) {
        ...
        for (int axis = 0; axis < 6; axis++) {
            accSample[X] = accSamples[axis][X] / 400 - accelerometerConfig()->accZero.raw[X];
            accSample[Y] = accSamples[axis][Y] / 400 - accelerometerConfig()->accZero.raw[Y];
            accSample[Z] = accSamples[axis][Z] / 400 - accelerometerConfig()->accZero.raw[Z];
...
# 474 "./src/main/sensors/acceleration.c"
    if (!accIsCalibrationComplete()) {
...
# 515 "./src/main/sensors/acceleration.c"
}
