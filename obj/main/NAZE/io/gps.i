...
extern _Bool gpsHandleNMEA(void);
extern _Bool gpsHandleMTK(void);
extern _Bool gpsHandleUBLOX(void);
extern _Bool gpsHandleI2CNAV(void);
extern _Bool gpsDetectI2CNAV(void);
extern _Bool gpsHandleNAZA(void);
...
static gpsProviderDescriptor_t gpsProviders[GPS_PROVIDER_COUNT] = {
    ...
    { GPS_TYPE_NA, 0, 0, ((void *)0), ((void *)0) },




    { GPS_TYPE_SERIAL, MODE_RXTX, 0, ((void *)0), &gpsHandleUBLOX },
...
const gpsConfig_t pgResetTemplate_gpsConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .provider = GPS_UBLOX, .sbasMode = SBAS_NONE, .autoConfig = GPS_AUTOCONFIG_ON, .autoBaud = GPS_AUTOBAUD_ON, .dynModel = GPS_DYNMODEL_AIR_1G, .gpsMinSats = 6 }
...
void gpsInit(void)
{
    ...
    if (gpsProviders[gpsState.gpsConfig->provider].type == GPS_TYPE_SERIAL) {
        ...
        if (!gpsPortConfig) {
            ...
            gpsState.gpsPort = openSerialPort(gpsPortConfig->identifier, FUNCTION_GPS, ((void *)0), baudRates[gpsToSerialBaudRate[gpsState.baudrateIndex]], mode, SERIAL_NOT_INVERTED);
...
# 289 "./src/main/io/gps.c"
void gpsFinalizeChangeBaud(void)
{
    if ((gpsProviders[gpsState.gpsConfig->provider].type == GPS_TYPE_SERIAL) && (gpsState.gpsPort != ((void *)0))) {

        if ((millis() - gpsState.lastStateSwitchMs) >= (200) && isSerialTransmitBufferEmpty(gpsState.gpsPort)) {
...
{
    return (epe > 99999) ? 9999 : epe;
}

uint16_t gpsConstrainHDOP(uint32_t hdop)
{
    return (hdop > 99999) ? 9999 : hdop;
}

void gpsThread(void)
{

    if (!isMPUSoftReset() && (millis() < (2000))) {
    ...
    if ((gpsProviders[gpsState.gpsConfig->provider].type == GPS_TYPE_SERIAL) && (gpsState.gpsPort != ((void *)0))) {
        ...
        case GPS_RECEIVING_DATA:
            gpsHandleProtocol();
            if ((millis() - gpsState.lastMessageMs) > (2000)) {
...
_Bool gpsMagInit(magDev_t *magDev)
{
    (void)(magDev);
    return 1;
}

_Bool gpsMagRead(magDev_t *magDev)
{
...
_Bool gpsMagDetect(magDev_t *mag)
{
