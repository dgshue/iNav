...
void mspSerialAllocatePorts(void)
{
    ...
    while (portConfig && portIndex < 3) {
        ...
        serialPort_t *serialPort = openSerialPort(portConfig->identifier, FUNCTION_MSP, ((void *)0), baudRates[portConfig->msp_baudrateIndex], MODE_RXTX, SERIAL_NOT_INVERTED);
        if (serialPort) {
...
static _Bool mspSerialProcessReceivedData(mspPort_t *mspPort, uint8_t c)
{
...
static mspPostProcessFnPtr mspSerialProcessReceivedCommand(mspPort_t *msp, mspProcessCommandFnPtr mspProcessCommandFn)
{
    ...
    mspPostProcessFnPtr mspPostProcessFn = ((void *)0);
...
void mspSerialProcess(mspEvaluateNonMspData_e evaluateNonMspData, mspProcessCommandFnPtr mspProcessCommandFn)
{
    for (uint8_t portIndex = 0; portIndex < 3; portIndex++) {
        ...
        while (serialRxBytesWaiting(mspPort->port)) {
            ...
            const _Bool consumed = mspSerialProcessReceivedData(mspPort, c);
